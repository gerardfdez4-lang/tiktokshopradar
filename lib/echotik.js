...
function normalizeVideo(v) {
  ...
  return {
    id: String(id),
    cover: v.cover_url || v.cover || v.origin_cover || null,
    url: v.share_url || v.video_url || (v.author_unique_id ? `https://www.tiktok.com/@${v.author_unique_id}/video/${id}` : `https://www.tiktok.com/video/${id}`),
    creator: v.author_unique_id ? `@${v.author_unique_id}` : (v.author_name || v.nickname || ""),
    views: v.play_cnt ?? v.play_count ?? v.views ?? 0,
    likes: v.digg_cnt ?? v.like_cnt ?? v.likes ?? 0,
  };
}

function normalizeCreator(c) {
  if (!c) return null;
  const handle = c.unique_id || c.author_unique_id || c.handle;
  return {
    name: c.nickname || c.author_name || c.name || handle || "Creator",
    handle: handle ? `@${handle}` : "",
    followers: c.follower_cnt ?? c.followers ?? c.fans_cnt ?? 0,
    avatar: c.avatar_url || c.avatar || null,
  };
}

// Vídeos + creators que promocionan el producto.
// NOTA: rutas/params a confirmar cuando haya cuota (plan Pro). Si fallan -> [] sin romper la ficha.
async function getProductMedia(productId, region) {
  const out = { videos: [], creators: [] };
  try {
    const v = await call("/api/v3/echotik/product/videolist", { product_id: productId, page_num: 1, page_size: 6, region });
    const list = Array.isArray(v) ? v : v?.list || [];
    out.videos = list.map(normalizeVideo).filter(Boolean);
  } catch { /* endpoint no disponible aún */ }
  try {
    const c = await call("/api/v3/echotik/product/influencerlist", { product_id: productId, page_num: 1, page_size: 8, region });
    const list = Array.isArray(c) ? c : c?.list || [];
    out.creators = list.map(normalizeCreator).filter(Boolean);
  } catch { /* endpoint no disponible aún */ }
  return out;
}
...
export async function getProductDetail(productId) {
  ...
  const [images, media] = await Promise.all([
    signCovers(rawImages),
    getProductMedia(p.product_id, p.region),
  ]);
  return {
    videos: media.videos,
